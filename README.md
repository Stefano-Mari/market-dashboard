...
- **Ingestion** (`stream_to_db.py`) - subscribes to live quotes and trade streams. A producer/consumer queue separates the socket callback from database writes, with batching set on a timer to reduce unnecessary writes.
...
- **API** (`main.py`) - FastAPI serving `/symbols`, `/bars/{symbol}`, `/metrics`, and `/quotes`.
...
- Run `python .\stream_to_db.py` and leave running to fetch live quotes and trades.

Open a new terminal and run the API server:
``` 
